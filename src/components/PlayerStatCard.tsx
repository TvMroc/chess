import { Box, Breadcrumbs, Card, CardContent, CardMedia, Divider, Typography } from "@mui/material";
...
export const PlayerStatCard = ({player, favorites, addFavorite, removeFavorite}: Props) => (
  <Card elevation={3} sx={{ minWidth: 300, width: '100%', opacity: 0.8, transition: 'opacity 0.3s', '&:hover': {opacity: 1} }}>
    <CardContent>
      ...
      <Box sx={{display:'flex', alignItems: 'center', gap: 1, justifyContent: 'space-between'}}>
        ...
        <DetailFavorite name={player.username.toLocaleLowerCase()} favorites={favorites} removeFavorite={removeFavorite} addFavorite={addFavorite}/>
      </Box>
    </CardContent>
  </Card>
)